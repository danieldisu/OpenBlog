(function(){
	...
	var listaPost = {
		...
		$.post('paneladmin/src/borrarPost.php', { idPost : idPost }, function(data){
			if(data){
				_this.reloadLista();
				$('.cajaAlertas').prepend('<div class="alert alert-success">Se ha eliminado correctamente el post</div>').fadeIn('slow');
			}else
				$('.cajaAlertas').prepend('<div class="alert alert-error">Ha habido un problema intentando borrar el post, vuelva a intentarlo mas tarde</div>').fadeIn('slow');
			});
			setTimeout(function(){
			  $('div .alert').fadeOut('slow');
			}, 4000);		
	...
	var estado = {
		...
		setEstadoEditandoPost : function(){
			...
			if(!volviendo){
				...
				$.ajax('paneladmin/src/getCategorias.php')
				).then(function(post, usuarios, categorias){
